...
import { fetchFrappeUsers, FrappeUser } from '../services/frappeAPI';
import {
  UserPlus, Search, Shield, ShieldAlert, ShieldCheck,
  Mail, Phone, Edit3, Trash2, X, Check,
  Key, Building, ClipboardCheck, AlertCircle, Loader2, RefreshCw
} from 'lucide-react';

function mapFrappeUser(fu: FrappeUser): User {
  const rolesArr = (fu.roles ?? []).map(r => r.role);
  let role = UserRole.STAFF;
  if (rolesArr.includes('System Manager') || rolesArr.includes('Administrator')) {
    role = UserRole.SUPER_ADMIN;
  } else if (rolesArr.includes('Projects Manager') || rolesArr.includes('Projects User')) {
    role = UserRole.ADMIN;
  }

  const permMap: Record<UserRole, Permission[]> = {
    [UserRole.SUPER_ADMIN]: ['view_dashboard', 'view_projects', 'add_project', 'edit_project', 'import_projects', 'manage_users', 'manage_feedback', 'manage_settings', 'delete_project'],
    [UserRole.ADMIN]: ['view_dashboard', 'view_projects', 'add_project', 'edit_project', 'manage_feedback'],
    [UserRole.STAFF]: ['view_dashboard', 'view_projects'],
    [UserRole.CITIZEN]: ['view_projects'],
  };

  return {
    id: fu.name,
    payrollNumber: fu.name,
    name: fu.full_name || fu.name,
    email: fu.email,
    phone: fu.mobile_no ?? '',
    role,
...
const UserManagement: React.FC = () => {
  const { data: frappeUsers, isLoading, isError, refetch } = useQuery<FrappeUser[]>({
    queryKey: ['frappe-users'],
    queryFn: fetchFrappeUsers,
    staleTime: 5 * 60 * 1000,
  });
  ...
  useEffect(() => {
    if (frappeUsers && frappeUsers.length > 0) {
      setUsers(frappeUsers.filter(u => u.enabled !== 0).map(mapFrappeUser));
    }
  }, [frappeUsers]);
    ...
    <div className="space-y-10 animate-fade-in pb-20">
      <div className="flex flex-col md:flex-row justify-between md:items-end gap-6">
        ...
        <div className="flex gap-3">
          <button
            onClick={() => refetch()}
            className="flex items-center justify-center gap-2 bg-white border border-slate-200 text-slate-600 px-5 py-4 rounded-2xl font-black shadow hover:bg-slate-50 active:scale-95 transition-all"
            title="Refresh"
          >
            <RefreshCw size={18} />
          </button>
      ...
      <div className="bg-white rounded-[3rem] border border-slate-200 shadow-2xl overflow-hidden">
        {isLoading && (
          <div className="flex items-center justify-center gap-3 py-24 text-slate-500 font-bold">
            <Loader2 size={22} className="animate-spin" />
            Loading staff from Frappe…
          </div>
        )}
        {isError && (
          <div className="flex items-center justify-center gap-3 py-24 text-rose-500 font-bold">
            <AlertCircle size={22} />
            Failed to load users. Check proxy connectivity.
          </div>
        )}
        {!isLoading && !isError && (
          <>
                      ...
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">ID: {u.payrollNumber}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-8 py-6">
      ...
      {/* User Form Modal — rendered via portal to escape transform containing block */}
